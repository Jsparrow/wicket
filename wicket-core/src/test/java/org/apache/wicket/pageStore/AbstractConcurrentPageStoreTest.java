...
public abstract class AbstractConcurrentPageStoreTest
{
	...
	private final ConcurrentLinkedQueue<SerializedPage> pagesToSave = new ConcurrentLinkedQueue<>();

	private final ConcurrentLinkedQueue<SerializedPage> filesToRead1 = new ConcurrentLinkedQueue<>();

	private final ConcurrentLinkedQueue<SerializedPage> filesToRead2 = new ConcurrentLinkedQueue<>();
	...
	protected void doTestStore(IPageStore pageStore)
	{
		...
		log.info(new StringBuilder().append("Took: ").append(duration).append(" ms").toString());
		log.info(new StringBuilder().append("Save: ").append(saveCount.intValue()).append(" files, ").append(bytesWritten.get()).append(" bytes").toString());
		log.info(new StringBuilder().append("Read: ").append(read1Count.get() + read2Count.get()).append(" files, ").append(bytesRead.get()).append(" bytes").toString());

		log.info("Average save time (ns): " + (double)saveTime.get() / (double)saveCount.get());

		assertEquals(0, failures.get());

		sessionCounter.keySet().forEach(s -> pageStore.removeAllPages(getContext(s)));
	}
