...
public class WicketObjects
{
	...
	public static <T> Class<T> resolveClass(final String className)
	{
		...
		{
			log.warn(new StringBuilder().append("Could not resolve class [").append(className).append("]").toString(), cnfx);
		}
		return resolved;
	}
	...
	public static <T> T cloneObject(final T object)
	{
		...
		else
		{
			...
			if (serialized == null)
			{
				throw new IllegalStateException(new StringBuilder().append("A problem occurred while serializing an object. ").append("Please check the earlier logs for more details. Problematic object: ").append(object).toString());
			}
	...
	public static void setObjectSizeOfStrategy(IObjectSizeOfStrategy objectSizeOfStrategy)
	{
		...
		log.info(new StringBuilder().append("using ").append(objectSizeOfStrategy).append(" for calculating object sizes").toString());
	}
