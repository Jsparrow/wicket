...
public class CheckingObjectOutputStream extends ObjectOutputStream
{
	...
	private static Method hasWriteReplaceMethodMethod;
	...
	static
	{
		try
		{
			...
			hasWriteReplaceMethodMethod = ObjectStreamClass.class.getDeclaredMethod(
					"hasWriteReplaceMethod", (Class[])null);
			hasWriteReplaceMethodMethod.setAccessible(true);
	 ...
	 */
	public CheckingObjectOutputStream(final OutputStream outputStream, final IObjectChecker... checkers) throws IOException
	{
	...
	private void internalCheck(Object obj)
	{
		...
		for (IObjectChecker checker : checkers)
		{
			...
			if (result.status == IObjectChecker.Result.Status.FAILURE)
			{
				...
				String exceptionMessage = new StringBuilder().append(result.reason).append('\n').append(prettyPrintMessage).toString();
		...
		for (;;)
		{
			try
			{
				...
				Class<?> repCl;
				if (!(Boolean)hasWriteReplaceMethodMethod.invoke(desc, (Object[])null) ||
						(obj = invokeWriteReplaceMethod.invoke(desc, obj)) == null ||
	...
	private static final class TraceSlot
	{
		...
		{
			return new StringBuilder().append(object.getClass()).append(" - ").append(fieldDescription).toString();
		}
	}
}
